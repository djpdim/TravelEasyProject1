        ...
        if (response.ok) {
            response.json().then(function (data) {
                ...
                for (var i = 0; i < forecastarray.length - 2; i++) {
                    ...
                    weatheri.attr(
                        "src",
                        "https://openweathermap.org/img/wn/" + forecastarray[i].weather[0].icon + ".png"
                    )
...
var getHotels = function (searchInput) {
    const options = {
        ...
        headers: {
            ...
            "X-RapidAPI-Key": "4235863ff5msh2bde6cc75dc7a15p1baab0jsnfa1b7a62f1c6",
        },
    }

    // Call the hotels.com API with input from the user search
    fetch(
        "https://hotels-com-provider.p.rapidapi.com/v1/destinations/search?query=" +
            searchInput +
